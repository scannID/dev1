...
import { ImagePlus, Search, Trash2, Loader2, Check } from 'lucide-react'
...
export function CatalogItemImageField({
  ...
  const preview = imageUrl || getCategoryImage(category || 'all')
  ...
  async function runSearch() {
    ...
    setSelectedId(null)
    try {
      const response = await imagesApi.search(q, 12)
  ...
  async function useResult(result: ImageSearchResult) {
    ...
    try {
      ...
      // Fallback: store the Pexels URL directly so the merchant can still use it
      try {
    ...
    <div className="grid gap-3">
      ...
      {searchOpen ? (
        <div className="rounded-xl border border-border bg-muted/20 p-4 grid gap-3">
          ...
          {results.length > 0 ? (
            <>
              <p className="text-xs text-muted-foreground">Tap a photo to use it on this item.</p>
            ...
            <p className="text-xs text-muted-foreground">
              Search for what you sell, tap a result to attach it, then save the item. You can still upload from your phone or computer.
            </p>
          )}
